...
\toprule
\textbf{Model (protocol)} & \textbf{macro-F1} & \textbf{MCC} & \textbf{ROC-AUC}\\
\midrule
Random Forest --- A0 (raw + random, source) & 0.9987 & --- & ---\\
Random Forest --- A1 (deduped + random) & 0.9984 & --- & ---\\
\midrule
Random Forest --- B0 (deduped + temporal) & 0.4883 & 0.2541 & 0.7735\\
XGBoost --- B1 (deduped + temporal) & 0.4972 & 0.2670 & 0.7425\\
Isolation Forest --- B2 (unsupervised) & 0.5473 & --- & ---\\
\bottomrule
\end{tabular}
\caption{Headline metrics across strategies. The two near-perfect rows use the flawed
protocol; the honest-protocol rows are far lower.}
\end{table}
