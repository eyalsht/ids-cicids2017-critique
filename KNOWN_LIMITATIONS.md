...
| 8 | **`src/ids_critique/` is imported directly by the notebook** (Section 0) — weekday parsing, `log1p`, `macro_f1`, and the flaw deltas all come from the package — so it is the real pipeline, not a parallel copy. EDA/plotting code remains inline by design (narrative clarity). | Low | The package is unit-tested (`tests/`, 16 tests) and is the canonical implementation of the stateless core. |
